...
DIR = Path(sys.argv[0]).parent.resolve() / "stylized_"
...
if __name__ == '__main__':
    ...
    content_path = input("Введите путь изображения ")
    style_path = input("Введите путь стиля ")
    while (not (load_file(content_path) and load_file(style_path))):
        content_path = input("Введите путь изображения ")
        style_path = input("Введите путь стиля ")
    ...
    np.save(DIR, content_image)
    np.save(DIR, style_image)
    np.save(DIR, stylized_image)
