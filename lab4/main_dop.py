...
frame_insert = cv2.VideoCapture("chessboard.mp4")
iw = int(frame_insert.get(cv2.CAP_PROP_FRAME_WIDTH))
ih = int(frame_insert.get(cv2.CAP_PROP_FRAME_HEIGHT))
...
try:
    while True:
        ...
        success_insert, insert = frame_insert.read()
        insert_pts = get_pattern(insert.shape)
        if not success_insert or not success_chess:
